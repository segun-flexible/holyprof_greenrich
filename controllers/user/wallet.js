...
const { getUserBank, getAllBank, verifyBankDetails, createNewBank, editBank, checkWalletAvailability } = require("../../helpers/bank");
...
exports.userWalletPost = asyncHandler(async (req, res, next) => {
    ...
    //Check Is A User Is Already Using The Account Number
    const numberAvailable = await checkWalletAvailability(id, req.body.number);

    if (numberAvailable) return res.json({ status: false, message: "Wallet Creation Failed", text: "Account Number Already Taken By An Investor" })
