...
//DEDUCT USER BALANCE
exports.deductUserBalance = async (userId,amountToDeduct,withdrawalType) => {
    return new Promise(async (resolve, reject) => {
        const user = await this.getUserById(userId);

        let query;

        if (withdrawalType === "ref") query = `UPDATE f_users SET referral_balance = ${user.referral_balance - amountToDeduct} WHERE uid = ?`
        else if(withdrawalType === "roi") query = `UPDATE f_users SET roi_balance = ${user.roi_balance - amountToDeduct} WHERE uid = ?`
        
        db.query(query, parseInt(userId), (err, user) => {
            if (err) reject(err)
            else resolve(user)
        })
    })
...
//GET TOP REFERRAL
exports.getTopReferral = (limit = 10) => {
    return new Promise((resolve, reject) => {
        db.query("SELECT * FROM f_users WHERE referral_total_count > 0 ORDER BY referral_total_count DESC LIMIT ?", limit, (err, user) => {
            if (err) reject(err)
            else resolve(user)
        })
    })
