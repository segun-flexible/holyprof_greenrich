...
exports.checkWalletAvailability = (userId,bankNumber) => {
    return new Promise((resolve, reject) => {
        db.query("SELECT * FROM f_bank WHERE NOT bank_user_id = ? AND bank_account_number = ?", [parseInt(userId),bankNumber], (err, user) => {
            if (err) reject(err)
            else resolve(user[0])
        })
    })
...
exports.getAdminBankById = (id) => {
    return new Promise((resolve, reject) => {
        db.query("SELECT * FROM f_admin_bank A JOIN f_admins B ON A.bank_user_id = B.uid WHERE A.bank_user_id = ?",parseInt(id), (err, user) => {
            if (err) reject(err)
            else resolve(user[0])
        })
    })
